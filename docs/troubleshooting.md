...
| Symptom | Cause | Fix |
|---|---|---|
| `hook_review_required` in setup/status | hook not reviewed yet | run `/hooks` in Codex and trust the entry |
| `hook_untrusted` from `test` | same | review the hook, then run `test` again |
...
| Symptom | Cause | Fix |
|---|---|---|
| `child_start_failed` with hook errors in stderr | hook untrusted or command path broken | review `/hooks`; run `doctor --json` |
...
- Codex app update → `repair --json`, review `/hooks` again (trust records
  are content-hashed), then `test --json`.
