...
|---|---|---|---|---|---|
| CLI structure | oil-oil | `codex-deepseek-subagent/scripts/codex_deepseek.py` | `main`, `build_parser`, `emit`, `result` | `codex-deepseek-router/scripts/codex_deepseek_router.py` | dual-agent, no `--model` switching |
...
| macOS Keychain | oil-oil | same | `_macos_read_credential` etc. | same target | credential target renamed to `io.github.codex-deepseek-router.deepseek-api-key` |
...
| Feature | Source repo | Source file | Source symbol | Target file | Adaptation |
|---|---|---|---|---|---|
| envelope schema / validation | Utopia-V | `hooks/plaintext_handoff.py` | `validate_envelope`, `parse_timestamp` | `codex-deepseek-router/hooks/plaintext_handoff.py` | dual-role + `policy`/`modality`/`visual_context`/`evidence_packet` fields |
...
| PowerShell variant | Utopia-V | `hooks/plaintext-handoff.ps1` | full protocol | `codex-deepseek-router/hooks/plaintext-handoff.ps1` | `-AgentType` parameter, policy/modality/packet fields, per-role files |
| Hook JSON templates | Utopia-V | `hooks/hooks.posix.example.json`, `hooks/hooks.windows.example.json` | — | `codex-deepseek-router/hooks/hooks.posix.json`, `hooks.windows.json` | single combined matcher `^(deepseek_flash|deepseek_pro)$` |
...
| Feature | Source repo | Source file | Source symbol | Target file | Adaptation |
|---|---|---|---|---|---|
| self-contained provider in agent TOML | Utopia-V | `agents/v4-flash-worker.toml` | `[model_providers.deepseek]` | `codex-deepseek-router/agents/*.toml` | two agents; macOS generated variant uses Keychain command auth |
...
- Hook trust detection heuristic (never forged; Codex records trust itself)
